...
**Phase 3 - result encoding.** Different parsers return different result types. `ParseMany[digit]` returns a list of characters; `ParseAction[p, f]` returns whatever `f` returns. FunctionCompile won't take a function that returns one of several types. The fix is *result-encoding* - pick one wide-enough type for the whole grammar and let each parser pack its result into it. For most grammars, a `Typed["GenericObject"]` (a managed Wolfram-expression handle) is the path of least resistance; for hot lexers, a packed-array-of-tokens form is faster.
