...
**Phase 3 - result encoding.** Different parsers return different result types. `ParseMany[digit]` returns a list of characters; `ParseAction[p, f]` returns whatever `f` returns. FunctionCompile requires a single, statically-known result type, so every parser must produce results of *one* compiled type. The fix is to encode all results as `"InertExpression"` - a managed handle to an ordinary Wolfram expression that flows through compiled code unchanged. Terminals pack their matched substring into one; combinators thread them; and - crucially - **`ParseAction`'s Wolfram callback compiles too**, via `Typed[KernelFunction[f], {"InertExpression"} -> "InertExpression"][...]`. `KernelFunction` lets compiled code call back into the kernel for exactly the arbitrary-Wolfram-function case actions need, so a grammar with semantic actions (including any `GrammarRules` rule) is fully compilable in a single pass - it does not have to drop to the interpreter.

This is what `ParserCompile` now does. The compiled function has the shape `(input :: "String", pos :: "MachineInteger") -> {finalPos, value}`: the position threads natively (the fast lexing path - `StringTake` / `DigitQ` / character-code ranges inline, no boxing per character), and the `value` is built as an `"InertExpression"` once per *matched node* (not per character) via the `KernelFunction` builders. `finalPos = -1` marks failure, on which the driver re-walks the interpreter once to recover the exact `ParseError` (failure is the rare path; success returns the real result directly). The whole non-recursive algebra lowers this way - `ParseSequence`, `ParseChoice`, `ParseMany`/`ParseSome`, `ParseBetween`, `ParseSepBy`/`ParseSepBy1`, `ParseChainLeft`/`ParseChainRight`, `ParseLookahead`/`ParseNotFollowedBy`, `ParseAction`, and the character classes (with a `KernelFunction`-wrapped `charMatchesQ` fallback for classes the compiler does not match natively). The mechanism is the one de-risked by `Tests/CompileFeasibility.wlt`; `Tests/Parser.wlt` checks that compiled output is identical to `Parse` across the algebra. The one combinator that does *not* lower is `ParseRecursive` (the compiler will not take the mutually-recursive function form the targets need); a parser that uses it stays on the interpreter, and `ParseMany` over a non-consuming parser is refused outright (`ParserCompile::infloop`).
...
The value-threading FunctionCompile codegen (Part 5) is now implemented: `ParserCompile` lowers the full non-recursive algebra to a single `CompiledCodeFunction` that returns the real parse result, with semantic actions run in place via `KernelFunction`. `ParseRecursive` stays on the interpreter on the default backend (the Wolfram Compiler will not take the mutually-recursive function form, and is too slow to FunctionCompile a large grammar - LaTeX lowers to one ~24k-node function).

## The PEG-VM backend

`ParserCompile[parser, Method -> "PEGVM"]` is the backend for large and recursive grammars. It abandons "compile each grammar to native code" - which is what makes FunctionCompile slow (superlinear in function size) and old `Compile` unusable (it cannot compile recursion; recursive calls fall back to uncompiled evaluation). Instead it compiles **one** thing, once: an LPEG-style parsing machine (a `Compile`d `While`-loop over a character-code array with an explicit backtrack/return stack - no language-level recursion, no strings, no Wolfram expressions in the hot loop). Each *grammar* is then lowered to a flat array of integer instructions - **data, not code** - that the one machine interprets.

The consequences are decisive:

- **Any grammar of any size "compiles" in plain Wolfram Language in milliseconds-to-seconds** (LaTeX is a ~380k-instruction table built in ~2-4s), with no per-grammar `FunctionCompile` and no recursion-depth limit on the compiler.
- **Recursion is free** - `ParseRecursive` is just a `Call`/`Return` opcode pair; the 258-non-terminal TPTP grammar and the 16-cycle LaTeX grammar both run.
...
- **Results are produced via captures.** The machine records `(nodeId, position)` open/close events on a backtrack-safe capture stack during the native run; a Wolfram post-pass turns those into a tree and rebuilds the exact result, applying the same `ParseAction` callbacks as the interpreter. `ParseChoiceLongest` is implemented as true longest-match (each alternative is measured, the furthest-reaching one is committed). Output is bit-identical to `Parse` on success.
- **It serializes.** Because the compiled form is an integer table plus a node-spec association, a PEG-VM parser can be `Export`ed and re-`Import`ed without recompiling (the machine itself recompiles lazily, once, on first use).

The trade-offs are an ASCII-only character-class table (literals stay full-Unicode) and generic `ParseError` messages on failure (the native run does not accumulate the expected-token set). For the worked targets - LaTeX math and TPTP - the PEG-VM is the path that actually delivers a fast, compiled, serializable parser.
